          ...
          <div className="flex flex-col gap-3 w-full mt-2">
            ...
            <button
              onClick={() => {
                ...
                // Short delay so the browser allows the download action right after the popup
                setTimeout(() => handleMergeAndDownload(), 500);
              }}
              disabled={isProcessing}
              className="w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-2xl text-white font-bold transition-all active:scale-[0.99] text-sm shadow-md bg-gradient-to-r from-primary to-pink-500 hover:opacity-95"
            >
              {isProcessing ? (
                <>
            ...
            {/* Option 2: Share to WhatsApp Status */}
            <button
              onClick={() => {
                const text = `I just joined the ${campaignTitle} campaign! Join me and generate your own frame here: ${window.location.href}`;
                window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(text)}`, "_blank");
            ...
            {/* Option 3: Download Poster Only */}
            <button
              onClick={handleMergeAndDownload}
              disabled={isProcessing}
              className={`w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-2xl font-bold transition-all active:scale-[0.99] text-sm shadow-sm ${
                isDownloaded
